layui.use(['form','layer','table','laytpl','laydate','jquery'],function(){
    ...
    var tableIns = table.render({
        elem: '#userList',
        url : '../../../biz/latereturn_findByPage.action',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limits : [10,15,20,25],
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            for (var i in data) {
                newsId.push(data[i].lateReturnId);
            }
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("../../../biz/latereturn_deleteBatch.action",{
    ...
    table.on('tool(userList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get("../../../biz/latereturn_delete.action",{
                    lateReturnId : data.lateReturnId  //将需要删除的newsId作为参数传入
                },function(data){
                    if (data.code === 0){
                        layer.msg("删除成功");
                    }else {
